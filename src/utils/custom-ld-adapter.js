...
function install (LocusZoom) {
    ...
    class LdServer extends BaseUMAdapter {  
        constructor(t) {
        t.limit_fields || (t.limit_fields = ["variant2", "position2", "correlation"]), super(t)
    }

     w(t, e = !1) {
        const v = /^(?:chr)?([a-zA-Z0-9]+?)[_:-](\d+)[_:|-]?(\w+)?[/_:|-]?([^_]+)?_?(.*)?/;
        const s = t && t.match(v);
        if (s) return s.slice(1);
        if (e) return null;
        throw new Error(`Could not understand marker format for ${t}. Should be of format chr:pos or chr:pos_ref/alt`)
    }
    __find_ld_refvar(t, e) {
        ...
        n.lz_is_ld_refvar = !0;
        const o = this.w(a, !0);
        if (!o) throw new Error("Could not request LD for a missing or incomplete marker format");
        const [r, l, h, c] = o;
        a = `${r}:${l}`, h && c && (a += `_${h}/${c}`);
        const d = +l;
        return d && t.ldrefvar && t.chr && (r !== String(t.chr) || d < t.start || d > t.end) ? (t.ldrefvar = null, this.__find_ld_refvar(t, e)) : a
    }
    _buildRequestOptions(t, e) {
        ...
        return "1000G" === a && "GRCh38" === i && (a = "1000G-FRZ09"), this._validateBuildSource(i, null), Object.assign({}, s, {
            genome_build: i,
            ld_source: a,
            ld_population: n
        })
    ...
    _getURL(t) {
        ...
        return [super._getURL(t), "genome_builds/", o, "/references/", r, "/populations/", l, "/variants", "?correlation=", e, "&variant=", encodeURIComponent(n), "&chrom=", encodeURIComponent(s), "&start=", encodeURIComponent(i), "&stop=", encodeURIComponent(a)].join("")
    }
    _getCacheKey(t) {
    ...
    _performRequest(t) {
        ...
        let s = {
            ...
            i = function (t) {
                return fetch(t).then().then((t => {
                    if (!t.ok) throw new Error(t.statusText);
                    return t.text()
                })).then((function (t) {
                    return t = JSON.parse(t), Object.keys(t.data).forEach((function (e) {
